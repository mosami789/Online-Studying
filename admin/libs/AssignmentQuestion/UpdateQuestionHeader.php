<?php 
 ...
 if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $selHeaderQue = $conn->query("SELECT * FROM assi_question_header WHERE que_id='$id'");
        $selHeader = $selHeaderQue->fetch(PDO::FETCH_ASSOC);

        if ($CheckPhoto == 'NoPhoto'){
            $updQuestion = $conn->query("UPDATE assi_question_header SET title='$title', que_text='$text'  WHERE que_id='$id' ");
            if($updQuestion)
            {
            ...
            if (in_array($uploaded_file_mime_type, $allowed_mime_types)) {
                ...
                if ($uploadOk == 0) {
                    ...
                    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                        ...
                        $updQuestion = $conn->query("UPDATE assi_question_header SET img='$photo_name'  WHERE que_id='$id' ");
                        if($updQuestion)
                        {
                            $res = array("status" => "success");
                        }else{
            ...
            if (in_array($uploaded_file_mime_type, $allowed_mime_types)) {
                ...
                if ($uploadOk == 0) {
                    ...
                    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                        ...
                        $updQuestion = $conn->query("UPDATE assi_question_header SET title='$title', que_Text='$text' , img='$photo_name'  WHERE que_id='$id' ");
                        if($updQuestion)
                        {
                            $res = array("status" => "success");
                        }else{

<?php 
 ...
 if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($CheckPhoto == 'NoPhoto'){
                $insQue = $conn->query("INSERT INTO exam_question_header(title, exam_id, Que_Text) VALUES('$title','$exam_id', '$text') ");
                if($insQue)
                {
                ...
                if (in_array($uploaded_file_mime_type, $allowed_mime_types)) {
                    ...
                    if ($uploadOk == 0) {
                        ...
                        if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                            $insQue = $conn->query("INSERT INTO exam_question_header(title, exam_id, Que_Text , img) VALUES('$title','$exam_id', '$text' , '$photo_name') ");
                            $res = array("status" => "success");
                        } else {

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($SelectCase == '1'){
        ...
        $updQuestion = $conn->query("UPDATE exam_question_header SET img=''  WHERE que_id='$id' ");
        if($updQuestion)
        {
        ...
        $delQueHeader = $conn->query("DELETE  FROM exam_question_header WHERE que_id='$id' ");
        if($delQueHeader)
        {   
            $query = $conn->query("DELETE FROM exam_question_tbl WHERE exam_header = '$id'");
